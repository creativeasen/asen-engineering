...
- Only repos listed in `asen-engineering-private/projects.md` with status `active`. **Never any client repo**, even if it appears elsewhere.
...
1. Read `projects.md` in `asen-engineering-private`. If it lists no active ASEN repos, write "No projects to audit" and stop.
...
3. Save the report to `asen-engineering-private/audit-reports/YYYY-MM-DD-<project>.md`: a 5-line summary for Aakash, a findings table (severity, file:line, problem, fix), and the top fixes in order.
4. For each **real** problem (not style nits, not guesses), open **one issue in that ASEN repo** labeled `severity:critical`, `severity:high`, `severity:medium`, or `severity:low`. Before creating, search open issues to avoid duplicates. Put no secrets in issues.
